...
fn emit<R: Runtime>(app: &AppHandle<R>, state: &'static str, app_name: String) {
    let url = (app_name == "Google Chrome").then(chrome_url).flatten();
    let _ = app.emit(
        "lockin",
        Transition {
            state,
...
fn chrome_url() -> Option<String> {
    osascript(r#"tell application "Google Chrome" to if (count of windows) > 0 then get URL of active tab of front window"#)
